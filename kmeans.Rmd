...
For the purposes of this workshop, we have no missing data. Follow this link to learn more about the [data](https://cran.r-project.org/web/packages/cluster.datasets/cluster.datasets.pdf).

What's in the data set:

```{r, eval=TRUE}
data("new.haven.school.scores")
new.haven.school.scores
```

The size of the dataset (n=`r nrow(new.haven.school.scores)`) for clustering depends on the availability of complete cases. We include all four numeric vectors of student scores in either reading or arithmetic. We create a data frame containing only these columns of interest. 
